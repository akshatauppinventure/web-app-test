...
| `wireguard/wg0-{edge,core}.conf.tmpl`, `wireguard/peers.yaml`, `scripts/wg-apply-psks.sh` | Interface templates (private key generated on the host at first boot, never leaves it), public-key peer map, PostUp hook applying pre-shared keys from `/etc/wireguard/psk-*` files pushed as secrets |
...
Provisioning order (T20): OpenTofu passes the rendered cloud-init as user data → first boot generates the WireGuard keys → the admin reads `/etc/wireguard/public.key` from both hosts, fills `wireguard/peers.yaml`, re-renders and updates `/etc/wireguard/wg0.conf` on each host (`wg syncconf`) → pushes secrets (T19) → starts the Portainer bootstrap project (T21).
