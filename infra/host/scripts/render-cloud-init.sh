...
# vars file (KEY=VALUE): HOSTNAME, ADMIN_USER, ADMIN_SSH_PUBKEY, PUBLIC_HOST, PUBLIC_IF,
#                        ADMIN_PUBLIC_KEY, CORE_PUBLIC_KEY/CORE_ENDPOINT (edge) or EDGE_PUBLIC_KEY/EDGE_ENDPOINT (core)
set -euo pipefail
...
. "$VARS"
set +a
: "${HOSTNAME:?}" "${ADMIN_USER:?}" "${ADMIN_SSH_PUBKEY:?}" "${PUBLIC_HOST:?}" "${PUBLIC_IF:?}" "${ADMIN_PUBLIC_KEY:?}"
...
subs = {
    ...
    "__CORE_PUBLIC_KEY__": env.get("CORE_PUBLIC_KEY", "REPLACE_AFTER_PROVISIONING"),
    "__CORE_ENDPOINT__": env.get("CORE_ENDPOINT", "10.0.0.2:51820"),
    "__EDGE_PUBLIC_KEY__": env.get("EDGE_PUBLIC_KEY", "REPLACE_AFTER_PROVISIONING"),
...
left = [l for l in text.splitlines() if "__" in l and "__WG_PRIVATE_KEY__" not in l]
if left:
