...
Second provider module with the **same contract** as `infra/tofu/upcloud` (checked by `scripts/test/tofu-contract.sh`): same variable and output names, same `terraform.tfvars` keys, same downstream steps (cloud-init render, SSH config, DuckDNS). Use it for the OVHcloud POC after the UpCloud one.

Creates exactly: 2 instances (`<prefix>-edge` d2-4, `<prefix>-core` d2-8, newest public `Ubuntu 26.04` image, Ext-Net public interface + vRack private interface at 10.0.0.11/.2), 1 keypair, 1 private network + 1 subnet (VLAN 42, DHCP, no gateway), 2 security groups with 6 (edge: 80/443, SSH from each `admin_ssh_cidrs` entry, ICMP echo, private network, egress) and 4 (core: SSH, ICMP echo, private network, egress) rules with the default single CIDR. Security groups are stateful, so no return-traffic rules; no IPv6 rules, so IPv6 is dropped as on UpCloud.
