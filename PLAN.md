...
_Version 1.2, 2026-09-19 (WireGuard removed, ADR-0026). Derived from the accepted decisions in [`docs/adr/`](docs/adr/README.md). Hostname: **`test-vinayak.duckdns.org`**. Repository: `akshatauppinventure/web-app-test`. Task status is tracked in `CLAUDE.md`._
...
4. ~~WireGuard bootstrap keys come from the laptop~~ **Obsolete 2026-09-19:** WireGuard was removed (deviation 6).
6. **WireGuard removed from the POC (ADR-0026, supersedes ADR-0015; 2026-09-19, owner decision).** Admin SSH is public on both hosts (keys only, rate-limited, optional source allowlist `ADMIN_SSH_CIDRS` / `admin_ssh_cidrs`). Edge-to-core traffic uses the provider private network (`10.0.0.11` / `10.0.0.2`) in plain HTTP. Portainer and the Keycloak admin console are reached through SSH port forwarding. Restoring a VPN is item 1 of [`docs/production-hardening.md`](docs/production-hardening.md); T24 reports the accepted risks.
...
| P7 | — | ~~Install WireGuard on your laptop; generate its key pair~~ — **no longer needed** (ADR-0026, 2026-09-19) | — |
...
### T17 · Host configuration: cloud-init, nftables, WireGuard, Docker daemon, timers — **L** ∥ (WireGuard parts removed 2026-09-19, ADR-0026)
...
- **Owner action:** P6, run `tofu apply` (billing starts). The servers created on 2026-09-17 use the WireGuard design and must be destroyed and re-created (`docs/runbooks/provisioning.md` step 0).
