...
- WireGuard port: `WG_PORT` (render) / `wireguard_port` (both modules), default 51820. The UpCloud POC runs on **33434** with `manage_provider_firewall = false` because the owner kept the account in trial mode (fixed firewall, PLAN deviation 5); the laptop config uses the same `ListenPort`.
